...
def main():
    ...
    import argparse
    ap = argparse.ArgumentParser(description="Process decrypted data into flat CSV")
    ap.add_argument("--input-dir", required=True, help="Directory containing decrypted *_分类.csv files")
    args = ap.parse_args()
    ...
    for name in file_stems:
        input_csv = os.path.join(base_dir, f"{name}_解密.csv")
        if not os.path.exists(input_csv):
            print(f"跳过 {name}: 找不到解密后的CSV")
            continue

        output_csv = os.path.join(base_dir, f"{name}_分类.csv")

        print("\n" + "=" * 80)
        print(f"开始分类处理: {name}")
        print("=" * 80)

        process_single_file(input_csv, output_csv)
