...
def main():
    ...
    # 文件路径配置 - 通过命令行参数或环境变量注入
    import argparse
    ap = argparse.ArgumentParser(description="Decrypt VIN info fields (v2 single file)")
    ap.add_argument("--input-dir", default=os.environ.get("VIN_INPUT_DIR", "./data"),
                    help="Directory containing input files")
    args = ap.parse_args()
    base_dir = args.input_dir
    
    # 可以选择从CSV或TXT文件读取
    input_file = os.path.join(base_dir, "提取的Info字段数据.csv")  # 或使用 "提取的Info字段数据.txt"
    output_txt = os.path.join(base_dir, "解密后的Info字段数据.txt")
    output_csv = os.path.join(base_dir, "解密后的Info字段数据.csv")
    
    # 检查输入文件
    if not os.path.exists(input_file):
        # 如果CSV不存在，尝试TXT
        input_file = os.path.join(base_dir, "提取的Info字段数据.txt")
        if not os.path.exists(input_file):
            print(f"错误：找不到输入文件")
            print(f"请确保以下文件之一存在：")
            print(f"  - 提取的Info字段数据.csv")
            print(f"  - 提取的Info字段数据.txt")
            return
    
    # 添加调试模式选项
    print("是否启用调试模式？(查看第一条记录的详细解密过程)")
    print("1. 是 - 调试模式")
    print("2. 否 - 直接处理所有记录")
    ...
    if choice == '1':
        ...
        if records:
            ...
            print(f"\n序列号: {test_record.get('seqNo', 'N/A')}")
            print(f"请求时间: {test_record.get('requestTime', 'N/A')}")
            
            fields = ['callSupplierRequestInfo', 'callSupplierResponseInfo', 'requestInfo', 'responseInfo']
            for field in fields:
                encrypted = test_record.get(field, '')
                if encrypted:
                    print(f"\n{'='*60}")
                    result = decrypt_field(field, encrypted, debug=True)
