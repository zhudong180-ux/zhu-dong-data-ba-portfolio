# -*- coding: utf-8 -*-
"""
用途：对汽车数据 Excel（.xlsx）逐行读取，按规则生成“清洗后车型名称”，并在末尾新增一列写回到新文件。
...
特点：
- 流式读取（openpyxl read_only）+ 流式写出（xlsxwriter constant_memory），适合大表（百万行级）
- 默认不引入表头（因为很多原表第一行就是数据），只是追加一列
- 自动从每行里通过“前缀字段”（年款：/排量：/变速器：/驱动：）+ “车型全称（包含‘款’）”做解析
...
用法：
    python clean_vehicle_name_v2.py --input 总2-1.xlsx --output 总2-1_清洗后.xlsx
可选：只处理前 N 行做验收
    python clean_vehicle_name_v2.py --input 总2-1.xlsx --output demo.xlsx --limit 10000
"""
...
def extract_trim(full_name: Optional[str],
    ...
    """
    从车型全称中提取“配置等级/版本”。
    做法：去掉年款、品牌车系前段、动力单元、变速箱、驱动后，剩下的合并为空格规范化文本。
    若车型全称本身不含版本（比如只有“... 手自一体”），则返回 None。
    """
    if not full_name:
        return None
...
def build_clean_name(cells: List[Any]) -> Optional[str]:
    ...
    # “变速箱/驱动形式 + 配置等级”合并成最后一段（不加空格，符合你给的示例）
    last = ""
    if trans:
        last += trans
    if drive:
...
def process_file(input_path: Path, output_path: Path, limit: int = 0) -> None:
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=0, help="只处理前N行用于验证（0=全量）")
    args = ap.parse_args()
    process_file(Path(args.input), Path(args.output), limit=args.limit)
