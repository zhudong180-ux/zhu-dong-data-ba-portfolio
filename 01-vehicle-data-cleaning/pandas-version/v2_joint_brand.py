...
def extract_brand_series(text):
    ...
    def combine_brand_series(brand, series):
        brand = brand.strip()
        series = series.strip()
    ...
    if '>' in text:
        ...
        if len(parts) >= 2:
            ...
            if '_' in series_part:
                ...
                return combine_brand_series(brand, car_series)
            else:
                brand_candidate = ""
                if len(parts) >= 3:
                    # 选取前面部分中去掉空格后最长的作为品牌
                    ...
                    if candidates:
                        brand_candidate = max(
                            candidates,
                            key=lambda p: len(re.sub(r'\s+', '', p))
                        )
                if not brand_candidate and len(parts) >= 2:
                    for candidate in reversed(parts[:-1]):
                        if candidate != series_part:
                            brand_candidate = candidate
                            break

                if '_' in brand_candidate:
                    brand_candidate = brand_candidate.split('_', 1)[0]

                result = combine_brand_series(brand_candidate, series_part)
                return result if result else series_part
