...
def extract_brand_series(text):
    ...
    engine_token = re.compile(r"\d+(?:\.\d+)?[TL]", re.IGNORECASE)
    ...
    def strip_engine_tokens(series: str) -> str:
        """Drop embedded displacement info so the车型 retains code like CS15."""
        match = engine_token.search(series)
        if match:
            return series[:match.start()].strip()
        return series.strip()
    ...
    if '>' in text:
        ...
        car_series = strip_engine_tokens(car_series)
...
def extract_transmission(text):
    """Return transmission text; include EV single speed."""
    if pd.isna(text):
        return ""
    text = str(text)
...
def _process_template(input_path, output_path, columns_to_join=None, sheet_name=None):
    sheet_label = sheet_name if sheet_name else '默认工作表'
    print(f"[INFO] Loading sheet: {sheet_label}")
    df_original = pd.read_excel(input_path, header=None, sheet_name=sheet_name)
    ...
    for idx, row in df_original.iterrows():
        ...
        if columns_to_join is None:
            ...
            raw_info = " ".join(str(row[i]) for i in columns_to_join if i < len(row))

        year_col = row[2] if len(row) > 2 and columns_to_join is None else ""
...
def process_file_1(input_path, output_path, sheet_name=None):
    _process_template(input_path, output_path, sheet_name=sheet_name)


def process_file_2(input_path, output_path, sheet_name=None):
    _process_template(input_path, output_path, sheet_name=sheet_name)


def process_file_3(input_path, output_path, sheet_name=None):
    _process_template(input_path, output_path, columns_to_join=range(1, 11), sheet_name=sheet_name)


def process_file_4(input_path, output_path, sheet_name=None):
    _process_template(input_path, output_path, columns_to_join=range(1, 10), sheet_name=sheet_name)


def main():
    import argparse
    ap = argparse.ArgumentParser(description="Multi-sheet vehicle name cleaner")
    ...
    ap.add_argument("--target-sheet", default=os.getenv("TARGET_ACCURACY_SHEET", "70%_1准确度"),
                    help="Target sheet name (or env TARGET_ACCURACY_SHEET)")
    ap.add_argument("--filename", default="cleaned_output.xlsx", help="Output filename")
    args = ap.parse_args()
    ...
    try:
        if os.path.exists(input_path):
            ...
            if target_sheet not in excel_file.sheet_names:
                print(f"[ERROR] Sheet '{target_sheet}' not found. Available sheets: {excel_file.sheet_names}")
                return

            df_check = pd.read_excel(input_path, header=None, nrows=1, sheet_name=target_sheet)
            num_cols = len(df_check.columns)
            print(f"[INFO] Detected {num_cols} columns in sheet '{target_sheet}'")

            if num_cols == 3:
                process_file_1(input_path, output_path, sheet_name=target_sheet)
            elif num_cols == 7:
                process_file_2(input_path, output_path, sheet_name=target_sheet)
            elif num_cols == 11:
                process_file_3(input_path, output_path, sheet_name=target_sheet)
            elif num_cols == 10:
                process_file_4(input_path, output_path, sheet_name=target_sheet)
            else:
