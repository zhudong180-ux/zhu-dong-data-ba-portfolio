...
class KitModel(nn.Module):
    ...
    def __batch_normalization(dim, name, **kwargs):
        ...
        if 'scale' in KitModel.__weights_dict[name]:
            layer.state_dict()['weight'].copy_(torch.from_numpy(KitModel.__weights_dict[name]['scale']))
        else:
            layer.weight.data.fill_(1)

        if 'bias' in KitModel.__weights_dict[name]:
            layer.state_dict()['bias'].copy_(torch.from_numpy(KitModel.__weights_dict[name]['bias']))
        else:
            layer.bias.data.fill_(0)

        layer.state_dict()['running_mean'].copy_(torch.from_numpy(KitModel.__weights_dict[name]['mean']))
        layer.state_dict()['running_var'].copy_(torch.from_numpy(KitModel.__weights_dict[name]['var']))
        return layer
    ...
    def __conv(dim, name, **kwargs):
        ...
        layer.state_dict()['weight'].copy_(torch.from_numpy(KitModel.__weights_dict[name]['weights']))
        if 'bias' in KitModel.__weights_dict[name]:
            layer.state_dict()['bias'].copy_(torch.from_numpy(KitModel.__weights_dict[name]['bias']))
        return layer
